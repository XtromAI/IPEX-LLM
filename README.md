...
## Automation Scripts
- `start-ollama-server.ps1`: Start Ollama server with Intel GPU environment variables and required DLL paths (uses global conda env)
- `update-ipex-llm.ps1`: Update IPEX-LLM in the global conda environment
- `update-ollama.ps1`: Update Ollama binaries in the global conda environment by running init-ollama.bat (requires Administrator)
...
- **Server not responding:** Check if `start-ollama-server.ps1` is still running. Restart it if needed.
